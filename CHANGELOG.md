...
- **`testing-principles` styleguide** (`src/doctrine/styleguides/shipped/testing-principles.styleguide.yaml`) — language-neutral. All 12 Test Desiderata as principles (Fast, Isolated, Repeatable, Self-validating, Timely, Thorough, Truthful, Inspiring, Exemplary, Specific, Predictive, Minimal). Testing Pyramid and Clear Test Boundaries as patterns with code examples. Quad-A test structure (Arrange/Assumption-check/Act/Assert). Over-mocking, shared mutable state, and implementation-detail assertions as anti-patterns. Synthesised from the craft-guidelines-library generic testing guidelines.
- **`test-minimisation` tactic** (`src/doctrine/tactics/shipped/test-minimisation.tactic.yaml`) — 7-step coverage-gated process for removing redundant tests without losing behavioral coverage. Distinguishes logical from code duplication, enforces disable-before-delete safety, and unconditionally protects regression and documentation tests.
- **`formalized-constraint-testing` tactic** (`src/doctrine/tactics/shipped/formalized-constraint-testing.tactic.yaml`) — 4 property-based contract patterns (round-trip symmetry, equality contract, hash-code alignment, sparse serialization). Language-agnostic; fills the gap between black-box integration testing and unit TDD for components with mathematical invariants.
- **DRG profile nodes and edges** (`src/doctrine/graph.yaml`) — adds `agent_profile:implementer`, `agent_profile:java-implementer`, and `agent_profile:python-implementer` nodes with `delegates_to` specialisation edges and `requires`/`suggests` connections to all directives, tactics, styleguides, toolguides, and procedures. `resolve_transitive_refs(graph, {agent_profile:java-implementer}, {requires, suggests})` now returns the full governance context with zero dangling references. Also adds nodes for `java-conventions`, `maven-review-checks`, `testing-principles`, `test-minimisation`, and `formalized-constraint-testing` with appropriate cross-artifact edges.
- **Reviewer profile doctrine** — 4 tactics for `Reviewer Renata` shipped and wired into the DRG: `test-to-system-reconstruction` (dual-agent scored evaluation of test-as-specification quality), `work-package-completion-validation` (pre-transition quality gate), `secure-design-checklist` (9 Saltzer & Schroeder principles applied at design phase), `code-documentation-analysis` (terminology extraction for boundary discovery). `agent_profile:reviewer` added to graph with full `requires`/`suggests` edges to all directives and tactics. Graph: 139 nodes, 317 edges.
