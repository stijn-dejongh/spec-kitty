...
class TestOfflineQueue:
    ...
    def test_mark_synced_empty_list(self, temp_queue):
        ...
        temp_queue.queue_event(
            {"event_id": "evt-001", "event_type": "Test", "payload": {}}
        )
...
class TestQueueStats:
    ...
    def test_single_event_stats(self, temp_queue):
        ...
        temp_queue.queue_event(
            {"event_id": "evt-001", "event_type": "WPStatusChanged", "payload": {}}
        )
    ...
    def test_top_event_types_ranking(self, temp_queue):
        ...
        for i in range(5):
            temp_queue.queue_event(
                {"event_id": f"a-{i}", "event_type": "TypeA", "payload": {}}
            )
        for i in range(3):
            temp_queue.queue_event(
                {"event_id": f"b-{i}", "event_type": "TypeB", "payload": {}}
            )
        temp_queue.queue_event(
            {"event_id": "c-0", "event_type": "TypeC", "payload": {}}
        )
    ...
    def test_top_event_types_limited_to_five(self, temp_queue):
        ...
        for i in range(7):
            temp_queue.queue_event(
                {"event_id": f"evt-{i}", "event_type": f"Type{i}", "payload": {}}
            )
    ...
    def test_oldest_event_age_from_past_timestamp(self, temp_queue):
        ...
        conn.execute(
            ...
            (
                "old-evt",
                "TestEvent",
                json.dumps({"event_id": "old-evt", "event_type": "TestEvent"}),
                old_ts,
            ),
        )
        ...
        conn.execute(
            ...
            (
                "new-evt",
                "TestEvent",
                json.dumps({"event_id": "new-evt", "event_type": "TestEvent"}),
                now,
            ),
        )
        conn.commit()
