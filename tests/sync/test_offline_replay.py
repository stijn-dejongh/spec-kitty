    ...
    def test_queue_events_with_complex_payloads(self, temp_queue):
        ...
        for i in range(50):
            event = {
                ...
                "payload": {
                    "nested": {
                        "deep": {"value": i, "list": [1, 2, 3], "string": f"data-{i}"}
                    },
                    "tags": ["tag1", "tag2", "tag3"],
                    "metadata": {"key": "value"},
                },
...
class TestReconnectionTriggersBatchSync:
    ...
    def test_reconnection_triggers_batch_sync(self, mock_post, temp_queue):
        ...
        mock_response.json.return_value = {
            "results": [
                {"event_id": f"evt-{i:06d}", "status": "success"} for i in range(50)
            ]
        }
    ...
    def test_multiple_reconnection_cycles(self, mock_post, temp_queue):
        ...
        result = batch_sync(
            temp_queue, "token", "http://localhost:8000", show_progress=False
        )
...
class TestBatchSyncThroughput:
    ...
    def test_batch_sync_throughput_1000_events(self, mock_post, temp_queue):
        ...
        mock_response.json.return_value = {
            "results": [
                {"event_id": f"evt-{i:06d}", "status": "success"} for i in range(1000)
            ]
        }
        mock_post.return_value = mock_response
...
class TestIdempotency:
    ...
    def test_idempotency_duplicate_events(self, mock_post, temp_queue):
        ...
        mock_response1.json.return_value = {
            "results": [
                {"event_id": f"evt-{i:06d}", "status": "success"} for i in range(10)
            ]
        }
        ...
        mock_response2.json.return_value = {
            "results": [
                {"event_id": f"evt-{i:06d}", "status": "duplicate"} for i in range(10)
            ]
        }
            ...
            "results": [
                {
                    ...
                    "status": "duplicate" if i < 10 else "success",
                }
                for i in range(20)
            ]
        }
...
class TestQueueSizeLimit:
    ...
    def test_queue_size_limit_enforced(self, temp_queue):
        ...
        for i in range(OfflineQueue.MAX_QUEUE_SIZE):
            result = temp_queue.queue_event(
                {"event_id": f"evt-{i}", "event_type": "Test", "payload": {}}
            )
        ...
        result = temp_queue.queue_event(
            {"event_id": "evt-overflow", "event_type": "Test", "payload": {}}
        )
    ...
    def test_queue_accepts_after_sync(self, temp_queue):
        ...
        result = temp_queue.queue_event(
            {"event_id": "evt-new", "event_type": "Test", "payload": {}}
        )
        assert result is True
