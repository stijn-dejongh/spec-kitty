...
@pytest.fixture
def mission_with_wp(mission_dir: Path) -> Path:
...
class TestMissionContextHeader:
    def test_contains_slug(self, mission_dir: Path) -> None:
        header = _mission_context_header("042-test-mission", mission_dir, "claude")
        assert "042-test-mission" in header

    def test_contains_agent(self, mission_dir: Path) -> None:
        header = _mission_context_header("042-test-mission", mission_dir, "claude")
        assert "claude" in header

    def test_contains_directory(self, mission_dir: Path) -> None:
        header = _mission_context_header("042-test-mission", mission_dir, "claude")
        assert str(mission_dir) in header
...
class TestReadWPContent:
    def test_reads_existing_wp(self, mission_with_wp: Path) -> None:
        content = _read_wp_content(mission_with_wp, "WP01")
        assert "WP01 Content" in content

    def test_missing_wp(self, mission_dir: Path) -> None:
        (mission_dir / "tasks").mkdir()
        content = _read_wp_content(mission_dir, "WP99")
        assert "not found" in content.lower()

    def test_missing_tasks_dir(self, mission_dir: Path) -> None:
        content = _read_wp_content(mission_dir, "WP01")
...
class TestWriteToTemp:
    ...
    def test_filename_includes_agent_and_mission(self) -> None:
        """Different agents/missions produce different filenames (no collisions)."""
...
class TestBuildPromptWP:
    def test_implement_prompt_structure(self, mission_with_wp: Path) -> None:
        repo_root = mission_with_wp.parent.parent
        text, path = build_prompt(
            action="implement",
            mission_dir=mission_with_wp,
    ...
    def test_review_prompt_structure(self, mission_with_wp: Path) -> None:
        repo_root = mission_with_wp.parent.parent
        text, path = build_prompt(
            action="review",
            mission_dir=mission_with_wp,
...
class TestBuildPromptTemplate:
    ...
    def test_template_prompt_has_header(self, mock_governance, mock_resolve, mission_with_wp: Path) -> None:
        # Mock the resolver to return a fake template
        mock_path = mission_with_wp / "fake-template.md"
        ...
        assert "042-test-mission" in text
    ...
    @patch("specify_cli.next.prompt_builder.resolve_command")
    def test_template_prompt_bootstrap_context_first_load(self, mock_resolve, mission_with_wp: Path) -> None:
        mock_path = mission_with_wp / "fake-template.md"
        mock_path.write_text("# Specify Template\nCreate spec.md.\n", encoding="utf-8")
        mock_result = MagicMock()
        mock_result.path = mock_path
...
class TestGovernanceContext:
    @patch("specify_cli.next.prompt_builder.resolve_governance")
    def test_governance_context_renders_resolution(self, mock_resolve, mission_dir: Path) -> None:
        ...
        text = _governance_context(mission_dir.parent.parent)
        assert "Governance:" in text
        assert "Template set: software-dev-default" in text
    ...
    @patch("specify_cli.next.prompt_builder.resolve_governance")
    def test_governance_context_handles_failures(self, mock_resolve, mission_dir: Path) -> None:
