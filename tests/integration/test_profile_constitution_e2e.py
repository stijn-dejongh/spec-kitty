...
def test_local_support_declarations_end_to_end(tmp_path: Path) -> None:
    ...
    with patch("specify_cli.cli.commands.constitution.find_repo_root") as mock_root:
        ...
        assert not (constitution_dir / "library").exists(), (
            "library/ directory must NOT be materialised on disk"
        )

        # ── Step 4: agents.yaml must NOT be generated ──
        assert not (constitution_dir / "agents.yaml").exists(), (
            "agents.yaml must NOT be generated"
        )
...
def test_local_support_additive_warning_when_overlapping_shipped_concept(tmp_path: Path) -> None:
    ...
    assert overlap_warnings, (
        "Expected an additive-overlap diagnostic when local file targets a shipped directive"
    )
