...
def test_collect_feature_summary_reports_metadata_issue(
...
def test_perform_acceptance_without_commit(
    feature_repo: Path, feature_slug: str
) -> None:
    from tests.utils import run_tasks_cli
    ...
    summary = acc.collect_feature_summary(
        feature_repo, feature_slug, strict_metadata=True
    )
...
def test_collect_feature_summary_encoding_error(
    feature_repo: Path, feature_slug: str
) -> None:
...
def test_assignee_still_required_for_active_lanes(
    feature_repo: Path, feature_slug: str
) -> None:
    ...
    run(
        ["git", "commit", "-am", "Move to for_review without assignee"],
        cwd=feature_repo,
    )
...
def test_required_fields_still_enforced(feature_repo: Path, feature_slug: str) -> None:
    ...
    front, body, padding = th.split_frontmatter(wp_path.read_text(encoding="utf-8"))
    lines_no_lane = [
        line for line in front.splitlines() if not line.startswith("lane:")
    ]
    wp_path.write_text(
    ...
    front, body, padding = th.split_frontmatter(wp_path.read_text(encoding="utf-8"))
    lines_no_agent = [
        line for line in front.splitlines() if not line.startswith("agent:")
    ]
    wp_path.write_text(
    ...
    lines_no_pid = [
        line for line in lines_with_agent if not line.startswith("shell_pid:")
    ]
    wp_path.write_text(
