...
class TestVersionReading:
    ...
    def test_version_format(self):
        ...
        try:
            ...
            pytest.fail(
                f"Version '{module_version}' is not a valid PEP 440 version: {exc}"
            )
...
class TestEdgeCases:
    ...
    def test_version_does_not_crash_on_import(self):
        ...
        assert result.returncode == 0, (
            f"Importing version should not crash: {result.stderr}"
        )
...
class TestVersionUpdateWorkflow:
    ...
    def test_pyproject_toml_version_readable(self):
        ...
        if match:
            ...
            try:
                ...
                pytest.fail(
                    f"pyproject.toml version '{pyproject_version}' is invalid: {exc}"
                )
            assert len(parsed.release) >= 3, (
    ...
    def test_version_not_imported_from_pyproject(self):
        ...
        assert (
            "version_utils" in init_content or "importlib.metadata" in init_content
...
class TestRegressionPrevention:
    ...
    def test_cli_reports_current_version_not_old(self):
        ...
        for old_ver in old_versions:
            if (
                metadata_version != old_ver
            ):  # Only check if we're not actually that version
