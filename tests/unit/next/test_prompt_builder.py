...
class TestWriteToTemp:
    def test_creates_file(self) -> None:
        path = _write_to_temp(
            "implement", "WP01", "test content", agent="claude", feature_slug="042-feat"
        )
        assert path.exists()
    ...
    def test_filename_includes_action_and_wp(self) -> None:
        path = _write_to_temp(
            "review", "WP02", "content", agent="codex", feature_slug="042-feat"
        )
    ...
    def test_filename_without_wp(self) -> None:
        path = _write_to_temp(
            "specify", None, "content", agent="claude", feature_slug="042-feat"
        )
    ...
    def test_filename_includes_agent_and_feature(self) -> None:
        ...
        p1 = _write_to_temp(
            "implement", "WP01", "a", agent="claude", feature_slug="042-feat"
        )
        p2 = _write_to_temp(
            "implement", "WP01", "b", agent="codex", feature_slug="042-feat"
        )
        p3 = _write_to_temp(
            "implement", "WP01", "c", agent="claude", feature_slug="043-other"
        )
        assert p1 != p2
        assert p1 != p3
        assert p2 != p3
...
class TestBuildPromptTemplate:
    ...
    def test_template_prompt_has_header(
        self, mock_governance, mock_resolve, feature_with_wp: Path
    ) -> None:
    ...
    @patch("specify_cli.next.prompt_builder.resolve_command")
    def test_template_prompt_bootstrap_context_first_load(
...
class TestGovernanceContext:
    @patch("specify_cli.next.prompt_builder.resolve_governance")
    def test_governance_context_renders_resolution(
        self, mock_resolve, feature_dir: Path
    ) -> None:
    ...
    @patch("specify_cli.next.prompt_builder.resolve_governance")
    def test_governance_context_handles_failures(
        self, mock_resolve, feature_dir: Path
    ) -> None:
