...
class TestWorkflowImplementInstructions:
    ...
    def test_implement_instructions_include_git_commit_step_1(
    ...
    def test_implement_instructions_include_git_commit_step_2(
        self, workflow_repo: Path
    ):
        ...
        assert (
            "IMPLEMENTATION COMPLETE" in prompt_content
            or "WHEN YOU'RE DONE" in prompt_content
        )
        ...
        with patch("subprocess.run") as mock_run:
            ...
            def git_command_side_effect(args, **kwargs):
                ...
                if "branch" in args and "--show-current" in args:
                    ...
                    return MagicMock(
                        returncode=0, stdout=f"feature/{feature_slug}-WP01\n", stderr=""
                    )
                elif (
                    "status" in args
                    and "--porcelain" in args
                    and "kitty-specs" in str(args)
                ):
            ...
            assert any(
                any(
                    ...
                    for keyword in ["uncommitted", "staged", "unstaged"]
                )
                for line in guidance
            ), f"No uncommitted/staged message in: {guidance}"
            assert any("git add -A" in line for line in guidance), (
                f"No 'git add -A' in: {guidance}"
            )
            assert any("git commit" in line for line in guidance), (
                f"No 'git commit' in: {guidance}"
            )
