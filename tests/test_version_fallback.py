...
def test_version_fallback_chain():
    ...
    assert version != "0.0.0-dev", (
        "Should get version from importlib.metadata or pyproject.toml"
    )
...
def test_get_version_with_mocked_metadata_failure():
    ...
    with patch("importlib.metadata.version", side_effect=Exception("Mock failure")):
        ...
        assert version != "0.0.0-dev", (
            "Should not use last-resort fallback when pyproject.toml exists"
        )
...
def test_get_version_with_all_failures():
    ...
    with patch("importlib.metadata.version", side_effect=Exception("Mock failure")):
        with patch(
            "specify_cli.version_utils.read_version_from_pyproject", return_value=None
        ):
            version = get_version()
