...
class TestToolguideRepository:
    def test_list_all_from_shipped(self) -> None:
        """Acceptance: list_all returns shipped toolguides."""
        repo = ToolguideRepository()
        toolguides = repo.list_all()
        assert len(toolguides) >= 1

    def test_get_by_id(self) -> None:
        """Acceptance: get() returns shipped powershell toolguide."""
        repo = ToolguideRepository()
        toolguide = repo.get("powershell-syntax")
        assert toolguide is not None
        assert toolguide.tool == "powershell"
        assert toolguide.guide_path.endswith("toolguides/shipped/POWERSHELL_SYNTAX.md")
    ...
    def test_save_raises_without_project_dir(self, sample_toolguide_data: dict) -> None:
        from doctrine.toolguides.models import Toolguide
