...
class TestShippedContractsExistAndValidate:
    ...
    @pytest.mark.parametrize("action", SOFTWARE_DEV_ACTIONS)
    def test_contract_has_at_least_one_step(
    ...
    @pytest.mark.parametrize("action", SOFTWARE_DEV_ACTIONS)
    def test_delegates_to_references_valid_kinds(
        self, repo: MissionStepContractRepository, action: str
    ) -> None:
        contract = repo.get_by_action("software-dev", action)
        assert contract is not None
        for step in contract.steps:
            if step.delegates_to is not None:
                assert step.delegates_to.kind in ArtifactKind, (
                    f"Step {step.id} in {action} delegates to invalid kind: "
                    f"{step.delegates_to.kind}"
                )
...
class TestImplementContractStructure:
    ...
    def test_has_workspace_step_with_paradigm_delegation(
    ...
    def test_has_status_transition_step(self, contract: MissionStepContract) -> None:
        transition = next(
            (s for s in contract.steps if s.id == "status_transition"), None
        )
