...
class TestParadigmRepository:
    def test_list_all_from_shipped(self) -> None:
        """Acceptance: list_all returns shipped paradigms."""
        repo = ParadigmRepository()
        paradigms = repo.list_all()
        assert len(paradigms) >= 1

    def test_get_by_id(self) -> None:
        """Acceptance: get() returns shipped test-first paradigm."""
        repo = ParadigmRepository()
    ...
    def test_save_raises_without_project_dir(self, sample_paradigm_data: dict) -> None:
        from doctrine.paradigms.models import Paradigm
