    ...
    if result.returncode == 0:
        ...
        if orphaned:
            ...
            for line in orphaned:
                ...
                if (
                    "centralized_detect_feature_slug" not in content
                    and "detect_feature(" not in content
                ):
                    pytest.fail(
    ...
    for file_path in files_to_check:
        ...
        if uses_feature_detection:
            ...
            has_centralized_import = (
                "from specify_cli.core.feature_detection import" in content
            )
...
def test_error_messages_mention_feature_flag(repo_root: Path):
    ...
    detection_module = (
        repo_root / "src" / "specify_cli" / "core" / "feature_detection.py"
    )
    content = detection_module.read_text()

    # Check for helpful error messages
    assert "--feature" in content, "Error messages should mention --feature flag"
    assert "SPECIFY_FEATURE" in content, (
        "Error messages should mention SPECIFY_FEATURE env var"
    )
...
def test_implement_module_backward_compatible(repo_root: Path):
    ...
    implement_file = (
        repo_root / "src" / "specify_cli" / "cli" / "commands" / "implement.py"
    )
...
def test_agent_commands_accept_feature_parameter(repo_root: Path):
    ...
    for file_path in agent_files:
        ...
        commands = re.findall(
            r"@app\.command\([^)]*\)\s+def\s+(\w+)\(", content, re.MULTILINE
        )
...
def test_all_feature_detection_paths_tested(repo_root: Path):
    ...
    test_file = (
        repo_root / "tests" / "specify_cli" / "core" / "test_feature_detection.py"
    )
