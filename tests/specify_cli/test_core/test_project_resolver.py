...
def test_locate_project_root_and_template_resolution(tmp_path):
    ...
    (project / ".kittify" / "missions" / "software-dev" / "templates").mkdir(
        parents=True
    )
    (project / ".kittify" / "templates").mkdir(parents=True)
    (
        project / ".kittify" / "missions" / "software-dev" / "templates" / "foo.txt"
    ).write_text(
        "mission template",
        encoding="utf-8",
    )
    (project / ".kittify" / "templates" / "foo.txt").write_text(
...
def test_resolve_worktree_awareness(tmp_path):
    ...
    worktree = (
        repo_root / ".worktrees" / feature_slug / "kitty-specs" / feature_slug / "tasks"
    )
    worktree.mkdir(parents=True)

    cwd_inside = worktree / "doing"
    cwd_inside.mkdir()
    resolved = resolve_worktree_aware_feature_dir(
        repo_root, feature_slug, cwd=cwd_inside
    )
    assert (
        resolved
        == repo_root / ".worktrees" / feature_slug / "kitty-specs" / feature_slug
    )
