...
class TestAliasResolution:
    ...
    def test_alias_resolution_count(self, tmp_path: Path) -> None:
        ...
        agg.aliases_resolved = sum(1 for f in agg.features for wp in f.wp_details if wp.alias_resolved)
...
class TestIdempotency:
    ...
    def test_marker_prevents_remigration(self, feature_with_wps: Path) -> None:
        ...
        events = read_events(feature_with_wps)
        marker_events = [e for e in events if e.reason and "historical_frontmatter_to_jsonl:v1" in e.reason]
