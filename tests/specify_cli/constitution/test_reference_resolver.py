...
from specify_cli.constitution.reference_resolver import (
...
# ---------------------------------------------------------------------------
# Test 4: Cycle between tactics — both resolved once, no infinite loop
# ---------------------------------------------------------------------------

def test_cycle_between_tactics_resolved_once():
    # T1 references T2, T2 references T1
    ...
    assert graph.tactics.count("t2") == 1
    assert graph.unresolved == []
    assert graph.is_complete is True
