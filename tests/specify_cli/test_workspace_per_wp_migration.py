...
def test_specify_template_updated():
    ...
    if ".worktrees" in content_lower:
        ...
        for line in lines_with_worktrees:
            ...
            is_deferred = "implement" in line_lower and (
                "later" in line_lower or "during" in line_lower
            )
...
def test_plan_template_updated():
    ...
    if "worktree" in content_lower:
        ...
        for line in lines_with_worktree:
            ...
            is_explaining_change = any(
                word in line_lower for word in ["old", "previously", "legacy", "0.10"]
            )
    ...
    found_indicators = [
        ind for ind in main_repo_indicators if ind.lower() in content.lower()
    ]
...
def test_implement_template_exists():
    ...
    has_wp_ref = (
        "wp##" in content_lower or "wp01" in content_lower or "example" in content_lower
    )
