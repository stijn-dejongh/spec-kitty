    ...
    def test_read_file_resilient_with_autofix(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            assert content == 'User\'s "test"', (
                f"Content should be sanitized, got: {content!r}"
            )
    ...
    def test_read_file_resilient_without_autofix(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            assert "spec-kitty validate-encoding" in error, (
                "Error should suggest fix command"
            )
...
"""
            ...
            if len(lanes["planned"]) > 0:
                ...
                if card.get("encoding_error"):
                    # Error card scenario
                    assert "⚠️" in card.get("title", "") or "Encoding Error" in card.get(
                        "title", ""
                    ), "Error card should indicate encoding problem"
                    assert "WP01" in card.get("title", ""), (
                        "Should mention work package ID"
                    )
                else:
