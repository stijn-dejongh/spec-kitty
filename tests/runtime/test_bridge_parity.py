...
def _build_missing_feature_dir(base: Path) -> tuple[Path, dict]:
    snapshot = _software_dev_snapshot(base)
    return snapshot, {"agent": "pedro", "mission_slug": "does-not-exist", "result": "success"}


def _build_run_start_failure(base: Path) -> tuple[Path, dict]:
...
def _build_discovery_issues_specify(base: Path) -> tuple[Path, dict]:
    snapshot = _software_dev_snapshot(base)
...
def _build_specify_guard_fail(base: Path) -> tuple[Path, dict]:
    snapshot = _software_dev_snapshot(base)  # no spec.md
    advance_to_step(snapshot, "042-parity-oracle", "software-dev", "specify")
    return snapshot, {"agent": "pedro", "mission_slug": "042-parity-oracle", "result": "success"}


def _build_specify_guard_pass(base: Path) -> tuple[Path, dict]:
...
def _build_plan_guard_fail(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
...
def _build_plan_guard_pass(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
...
def _build_tasks_union_guard_fail_reqmap(base: Path) -> tuple[Path, dict]:
...
def _build_tasks_union_guard_fail_missing_deps(base: Path) -> tuple[Path, dict]:
...
def _build_tasks_union_guard_pass(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(
        snapshot,
...
def _build_implement_wp_remains(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(
        snapshot,
...
def _build_review_wp_remains(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(
        snapshot,
...
def _build_review_guard_pass_advances_accept(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(
        snapshot,
...
def _build_terminal(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(
        snapshot,
...
def _build_research_scoping_guard_fail(base: Path) -> tuple[Path, dict]:
...
def _build_research_gathering_guard_pass(base: Path) -> tuple[Path, dict]:
    mission_slug = "050-research-parity"
    snapshot = base / "snapshot"
...
def _build_research_output_guard_pass(base: Path) -> tuple[Path, dict]:
    mission_slug = "050-research-parity"
    snapshot = base / "snapshot"
    scaffold_research(
        snapshot,
...
def _build_documentation_discover_guard_fail(base: Path) -> tuple[Path, dict]:
...
def _build_documentation_generate_guard_pass(base: Path) -> tuple[Path, dict]:
    mission_slug = "060-docs-parity"
    snapshot = base / "snapshot"
...
def _build_query_finalized_override_implement(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(
        snapshot,
...
def _build_query_finalized_override_done(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(
        snapshot,
...
def _build_query_initial_fresh_mission(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = _software_dev_snapshot(base, mission_slug=mission_slug)
    return snapshot, {"agent": "pedro", "mission_slug": mission_slug}


def _build_query_runtime_after_advance(base: Path) -> tuple[Path, dict]:
    mission_slug = "042-parity-oracle"
    snapshot = base / "snapshot"
    scaffold_software_dev(snapshot, mission_slug, with_spec=True)
    advance_to_step(snapshot, mission_slug, "software-dev", "specify")
...
def _build_answer_input_mission(base: Path) -> tuple[Path, dict]:
    ...
    _commit_all(snapshot, "seed answer fixture")
    # Issue the first (and only) real step -- collect_input -- so a real
    # decision_required is pending before the answer-path fixture answers it.
    # NOTE: a decision_required does NOT set snapshot.issued_step_id (it is
    # carried in snapshot.pending_decisions instead), so advance_to_step's
    # issued_step_id-based loop cannot be reused here -- one direct real
    # engine call is enough to reach the pending decision (never stubbed).
    from runtime.next._internal_runtime import NullEmitter
    from runtime.next._internal_runtime import next_step as runtime_next_step
    from runtime.next.runtime_bridge import get_or_start_run as _get_or_start_run

    run_ref = _get_or_start_run(mission_slug, snapshot, "input-mission")
...
def _drive(spec: FixtureSpec, snapshot_dir: Path, drive_kwargs: dict, tmp_path: Path, run_label: str) -> FixtureRun:
...
def test_every_fixture_pair_is_parity_stable(
    ...
    for fixture_id, (run_a, run_b) in results.items():
        if run_a.entry == "answer_decision_via_runtime":
            ...
            continue
        try:
            assert_parity(run_a.decision, run_b.decision, run_a.repo_root)
...
def test_captured_side_effects_are_binding_equal(
    ...
    """Captured side effects (sync emit / coord commit / IC-02 engine mutations)
    must carry the same *shape* (method-name sequence) across two independent
    runs of the same fixture -- binding equality on WHAT was emitted/committed,
    not merely that something was.
    """
    results, _ledger = ledger_results
    failures: list[str] = []
    for fixture_id, (run_a, run_b) in results.items():
        names_a = [call[0] for call in run_a.side_effects.sync_emitter_calls]
        names_b = [call[0] for call in run_b.side_effects.sync_emitter_calls]
        if names_a != names_b:
            failures.append(f"{fixture_id}: sync emitter call sequence diverged: {names_a} vs {names_b}")
        coord_a = [call[0] for call in run_a.side_effects.coord_commit_calls]
        coord_b = [call[0] for call in run_b.side_effects.coord_commit_calls]
        if coord_a != coord_b:
            failures.append(f"{fixture_id}: coord commit call sequence diverged: {coord_a} vs {coord_b}")
    assert not failures, "side-effect shape breaks:\n" + "\n".join(failures)
...
def test_answer_path_side_effects_captured(
    ...
    """
    run_a, run_b = None, None
    for spec in FIXTURES:
        if spec.fixture_id == "answer_input_mission":
            break
    results, _ledger = ledger_results
...
def test_coverage_floor_is_met(
    ...
    Floor values are the ACTUAL counts this fixture ledger reaches on
    unmodified source (verified empirically while authoring this suite -- see
    the WP01 completion report for the full reached/unreached breakdown
    against the 29-site / guard-branch inventories). This is intentionally a
    checkable count, not a tautology: test_hollow_ledger_fails_coverage_floor
    (below) proves the same assertion helper fails on a sparse ledger.
    """
    _results, ledger = ledger_results
    assert_coverage_floor_met(ledger, site_floor=17, branch_floor=18)
