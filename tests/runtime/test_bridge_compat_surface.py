...
def _scenario_query_decision_required(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = _scaffold_project(tmp_path, "042-compat-guard")
    feature_dir = repo_root / "kitty-specs" / "042-compat-guard"
    _write_complete_artifacts(feature_dir, wp_lane="done")
    # Advance past the first step so the run has completed_steps -> the
    # "initial query" fast-path is bypassed and the decision_required branch
    # is reachable. A single decide_next call only ISSUES the first step
    # (completed_steps is still empty); two calls are needed so the first
    # step is actually completed before we query.
    rb.decide_next_via_runtime("test", "042-compat-guard", "success", repo_root)
    rb.decide_next_via_runtime("test", "042-compat-guard", "success", repo_root)
...
_SCENARIOS = {
    ...
    "query_decision_required": _scenario_query_decision_required,
...
REACH: dict[str, tuple[str, ...]] = {
    ...
    "_resolve_runtime_contract_for_step": ("happy_loop",),
    # Research-mission composed guard branches only (software-dev's guard
    # family never calls these two) — see the dedicated research_loop scenario.
    ...
    "_build_initial_query_decision": ("query_fresh_ephemeral",),
    "_build_decision_required_query": ("query_decision_required",),
    "_build_runtime_query_decision": ("query_existing",),
}
...
def test_reach_map_covers_the_full_grep_derived_inventory() -> None:
    ...
    """
    covered = set(REACH) | set(UNREACHABLE_FROM_CANONICAL_ENTRIES)
...
ALL_COMPAT_SYMBOLS: tuple[str, ...] = tuple(sorted(set(REACH) | set(UNREACHABLE_FROM_CANONICAL_ENTRIES)))
...
def test_guard_b_identity_reexport_for_relocated_symbols() -> None:
    ...
    On unmodified source every one of the 50 symbols is still natively
    defined in runtime_bridge.py (nothing has been relocated yet), so this
    currently has nothing to assert against -- exactly per the WP02 T009
    instruction ("author it so it becomes load-bearing as WP03+ relocate
    symbols"). The machinery is proven by construction: it walks ALL_COMPAT_
    SYMBOLS and would catch a future re-export that returns a copy instead of
    the seam's actual object.
    """
    import sys
    ...
    for name in ALL_COMPAT_SYMBOLS:
        ...
        if origin_module_name is None or origin_module_name == rb.__name__:
            continue  # still natively defined in runtime_bridge.py -- no-op today
        relocated_checked += 1
    ...
    # Documented no-op today; becomes load-bearing (relocated_checked > 0)
    # starting with WP03.
    assert relocated_checked == 0, (
        "Symbols have been relocated since this comment was written -- good, "
        "the identity check above is now load-bearing. Update this assertion "
        "to `relocated_checked > 0` once WP03+ lands."
    )
