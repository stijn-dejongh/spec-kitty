...
| Doctrine | YAML artifacts in `src/doctrine/` | Same — knowledge artifacts |
...
1. **Doctrine is a root dependency** — consumed by Constitution and Agent Tool Connectors; depends on nothing.
2. **Constitution depends only on Doctrine** — never on Kitty-core, Orchestration, or Event Store.
3. **Event Store is a shared persistence boundary** — writers (Kitty-core, Orchestration) and readers (Dashboard, Orchestration) interact through interface contracts, never directly with each other through the store.
4. **Dashboard has no write path** — strictly read-only against Event Store.
5. **Agent Tool Connectors are leaf nodes** — they execute work and consume governance context; they do not write to other containers except through Orchestration (results/events flow back through Orchestration to Event Store).
6. **Orchestration does not bypass Kitty-core** — it executes the graph that Kitty-core produced; it does not construct planning artifacts.
7. **Control Plane is the single user entry point** for mutations — Dashboard is read-only.
