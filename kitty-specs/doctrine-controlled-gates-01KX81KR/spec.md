...
| FR-002 | **Both** runtime gate consumers — the `move-task` pre-review hook AND the composed-action post-task guard (`_check_composed_action_guard`) — resolve the active gate set for a transition through a **single shared binding-resolver seam** (one injected port, not two parallel resolvers), replacing their hardcoded per-lane decision of which check runs. | Proposed |
...
| FR-006 | Gate bindings (and thereby their handler/asset targets) participate in **charter activation** so the active doctrine selects them; binding through activatable step contracts is the vehicle (assets are non-activatable directly today). | Proposed |
| FR-007 | Executing a **Path-B gate asset** is confined to a **trust envelope**: (a) provenance allowlist — built-in and org-pack only by default, **provenance derived from pack-load metadata, never self-declared by the asset**; (b) an explicit opt-in flag `review.allow_executable_gate_assets` (default **off**); (c) an interpreter allowlist with no shell interpolation; (d) an enforced timeout. A gate asset failing any check is **not executed**. | Proposed |
...
| FR-014 | Define a **canonical verdict→operator-outcome mapping** the whole system obeys: **BLOCK** only when a gate emits a valid `regression` verdict with `blocking=true`; **FAULT-WARN** for any FR-010 fault; **CALM-NOTICE** when nothing is declared/active (FR-008); **TRUST-REFUSAL** when FR-007 refuses execution; **PASS** on a valid non-regression verdict. No other input may block a transition. | Proposed |
| FR-015 | Executing a Path-B gate asset must be **contained**, not merely allow-listed: filesystem access confined to the mission/repo working tree (no writes outside it), **no outbound network egress**, and enforced memory/CPU/output-size limits in addition to the FR-007 timeout. If a required containment primitive is unavailable on the host, the asset is **not executed** (TRUST-REFUSAL), never run unconfined. | Proposed |
...
| NFR-006 | Bounded, contained execution (Path B). | Every gate-asset run is subject to timeout + FR-015 containment; an over-running or containment-violating run is terminated and folds to FAULT-WARN/TRUST-REFUSAL. Verified by a run that attempts network egress and a run that exceeds the timeout. | Proposed |
...
| C-007 | The new executable-asset runner is an untrusted-input sink: the existing `tests/architectural/untrusted_path_audit/` harness must be extended to cover it (no stale-green), reusing the `run_scoped_tests_at_head` argv/no-shell/timeout/env-scrub precedent. | Active |
...
| SC-007 | A Path-B gate asset attempting network egress or an out-of-tree write is contained/terminated and folds to FAULT-WARN/TRUST-REFUSAL — it does not succeed. |
...
- Splitting the `tasks_move_task.py` / `runtime_bridge.py` god-modules beyond the pre-review-hook extraction this mission needs — broader degod is #2116.
