...
- **Affected surfaces**: `egress.py` (a single internal decider computes the `EgressConsent` once and returns both the byte-identical refusal string and the state; `project_egress_refusal` becomes a thin `str | None` wrapper over it), `tracker/egress_verdict.py` (`_resolve_channel1` consumes the decider's `(permits, refusal_string, channel1_state)`; `_classify_channel1` and its independent routing/consent resolution deleted).
- **Sequencing/depends-on**: IC-01.
- **Risks**: the crux (post-spec M2) — the state must reach `egress_verdict` without a second resolution and without perturbing `project_egress_refusal`'s byte string. The thin-wrapper design keeps the existing `str | None` contract for the widen SaaS transport.
...
- **Risks**: an unmapped new member falling through to `_UNRECOGNISED_VERDICT_TEMPLATE` would change the hosted string (NFR-002 break) though not widen the seam. `saas_client`'s refusal string is not covered by NFR-002's pin — verify it explicitly.
