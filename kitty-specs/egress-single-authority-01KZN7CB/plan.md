...
- **Affected surfaces**: `egress.py` — a single internal decider `_egress_decision` **obtains** the `EgressConsent` member via the existing `resolve_egress_consent` seam (it performs **no** local consent/routing resolution and adds **no** `sync.consent`/`sync.routing` import — the C-003/C-005 invariant; the split-mapping lives once, in the resolver, per research Decision 1) and derives `(permits, refusal_message, channel1_state, generic)` from it; `project_egress_refusal` becomes a thin `str | None` wrapper over it. `tracker/egress_verdict.py` — `_resolve_channel1` consumes the decider's tuple; `_classify_channel1` and its independent routing/consent resolution are deleted; `_channel1_report`'s `(state, generic)` production is **absorbed into the decider** and its generic-rendering path is re-sourced (not deleted).
