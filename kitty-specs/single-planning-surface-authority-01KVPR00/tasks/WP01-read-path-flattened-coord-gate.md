...
## Context (live-evidence, #2062)
`_resolve_existing_for_slug` (`src/specify_cli/missions/_read_path_resolver.py:~270`) returns the
coord dir **unconditionally** on `CoordState.MATERIALIZED` (`return coord_feature_dir(...)`), with
no `_declares_coordination_branch` gate. `probe_coord_state` decides MATERIALIZED by pure
`Path.exists()`. So for a flattened mission the read-path leg leaks STALE-COORD for the
`<slug>-<mid8>` / bare-mid8 / full-ULID handles (surface + aggregate legs already gate on
`coordination_branch is None → primary`). See `quickstart.md` R1.

## Subtasks
### T001 — Gate MATERIALIZED on declared coordination (FR-004)
Thread a `declares_coordination: bool` signal (read once from primary `meta.json`) into
`_resolve_existing_for_slug` so the coord-preference branch (`coord_state is MATERIALIZED → return
coord_feature_dir`) only fires when `declares_coordination` is True. When a mission is flattened
(no `coordination_branch`), MATERIALIZED is necessary-but-not-sufficient → fall through to PRIMARY.
Match the create-window (#1718) and coord-deleted (#1848) behavior already in the surface leg — do
NOT regress those. Read the primary meta via the existing blessed helper (no raw join).
...
pass the same signal. Update the stale comment at ~`:263` ("No branch is supplied here") which
documents the defect as intentional.
...
Mutation-sensitive: it must FAIL with the T001 gate reverted.
