...
> **REMEDIATION APPLIED 2026-07-29 (post-compact).** All BLOCKERs (B1-B3), majors (M1-M8), minors (m1-m7), and escalations (E1 fold #2970 into IC-08; E2 dashboard dropped, #3068 filed) landed in `plan.md` (IC-map rewrite: IC-05→05a/b/c, IC-06→06a/b/c, Lane-B scope corrected per D-6, IC-08→IC-01 hard edge, #2274 blast radius, m4/m5 slug citations), `spec.md` (C-008/FR-002/edge-case consumer lists corrected; C-006 +#3068; C-007 two→three contracts; #2970 traceability row), and `contracts/` (write-seam-adoption.md B1 by-design + non-vacuity invariant; new `merge-driver-algorithm.md` M5; coord-authority-gate.md by-design + Move-B-is-ADR-amendment + slug). Ready for `/spec-kitty.tasks`.
