...
Make an execution lane branch from the **recorded planning-artifact commit** (the finalize-tasks tip captured in `lanes.json`/`meta.json`), not the pre-planning coord tip nor a moving `target_branch` tip, so matrix/tracer/planning writes share a common ancestor with the consolidation base and are **not reverted at merge** (#2993). This is a P0 git-topology change and is **ADR-first**.
