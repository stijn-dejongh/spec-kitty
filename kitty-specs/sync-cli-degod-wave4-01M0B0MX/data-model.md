...
| `AuthorityGate` — **read** port | adapter (new) | Coord/owner READ authority. |
| `AuthorityGate` — **write** port | adapter (new) | share/unshare/opt-in/opt-out WRITE authority. **Distinct from the read port; never unified (INV-2).** |
...
- **INV-2 (two-authority)**: read-authority and write-authority are distinct ports; no shared authority class (arch-test-guarded).
