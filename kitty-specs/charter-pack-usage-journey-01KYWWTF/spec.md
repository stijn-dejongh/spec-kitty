...
| FR-007 | Retire the legacy catalog-fallback in `resolve_project_governance` (`resolver.py:289`; `_resolve_directives_selection` returns `sorted(doctrine_catalog.directives)` at `:258-260`): when the authored governance selection is empty, source directives from the **config-activated set** — `PackContext.from_config(repo_root).activated_directives` (repo_root is already available; the seam currently builds an *unfiltered* catalog at `:316` and constructs no `PackContext`) — never the full catalog. **Structural root (confirmed):** the activation-aware `DoctrineService` wrapper (`resolver.py:57-139`) filters paradigms/procedures/agent_profiles by `PackContext` but has **no `directives` property**, so directives alone fall through to the unfiltered fallback (reproduced live: apply-5 + compile → 29, while `PackContext.activated_directives` reads exactly 5). FR-007 is that missing directives filter. | Draft |
...
| FR-010 | Make the advertised charter section selectors resolve (**#3095/#3094**): `charter context --include section:terminology-canon` / `section:code-review-checklist` (required by generated prompts — `implement/prompt.md`, `review/prompt.md`) resolve to their section — OR the surface stops advertising selectors it cannot resolve. **True target (post-M1 re-anchor):** the resolution engine is `context_renderers/section_bodies.py::render_critical_section_include` (`:282`; slugifies the `ACTION_CRITICAL_SECTIONS` headings incl. `TERMINOLOGY_CANON="Terminology Canon"` / `CODE_REVIEW_CHECKLIST="Code Review Checklist"`) reached via `build_charter_context_include`'s `kind=="section"` branch (`context.py:309`, error at `:354`) — **not** `context.py:397`. **The dead-end is a CONTENT question:** does the *compiled* charter contain those headings for `_extract_section_body` to find? Plan-phase decides "produce the sections in the compiled charter" vs "stop advertising". This stays on the `charter.md`/section *prose* path (C-003) — it does not fold that reader into the presence-gate retarget. | Draft |
