...
| FR-002 | Inject the resolver at the resolution **shell** `_assemble_core_fragments` (and sibling shell entries) via `resolver: MissionResolver \| None = None` → `resolver or FsMissionResolver()`. `build_execution_context` stays FS-free and takes no resolver. No adapter is placed on the frozen `MissionExecutionContext`. | Draft |
| FR-003 | Route the two additional resolve-by-identity consumers (`doctrine_synthesizer/apply.py:602/788`, `core/vcs/detection.py:169`) through the resolver in the same adoption WP. | Draft |
...
|----|-------------|---------------------|--------|
| NFR-001 | Builder FS-free-testability | ≥1 unit test constructs a valid `MissionExecutionContext` via `FakeMissionResolver` with **no** `kitty-specs/` tree present (runs green with the specs dir absent/empty). | Draft |
