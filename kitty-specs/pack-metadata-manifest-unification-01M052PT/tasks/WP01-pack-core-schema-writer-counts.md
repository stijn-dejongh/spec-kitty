...
### T004 — Built-in manifest generator
Create `src/specify_cli/doctrine/builtin_manifest.py`: generate `packs/built-in/pack-manifest.yaml` from the per-kind `packs/built-in/*.graph.yaml` `nodes:`. **Emit ONLY `pack-manifest.yaml`; never write `pack.yaml`** (that authored path is reserved for WP04 — this writer-boundary is a WP01 acceptance criterion). Wire into the build/upgrade path. Test: `test_builtin_manifest.py` (100% of DRG nodes enumerated — SC-002).

### T005 — Deterministic generation
Constituents sorted by `(kind, id)`; `content_hash` over **LF-normalized** artifact bytes (cross-platform, DIR-001); **exclude `generated_at`/`generated_by` from both the `manifest_hash` and the byte-diff assertion**. Test: generate twice for an unchanged pack → byte-identical file + identical `manifest_hash` (NFR-003).
...
The **pack** per-kind counts consumers are `pack_assembler.py:388-396`, `charter/_profile_health_render.py:111`, `_doctrine_collect.py:427`. Add `test_counts_derivation.py` proving the derived view returns identical per-kind values for each. **Explicitly out of scope:** the `dossier` `{total,required,required_present}` counts (`dossier/api.py:237`, dashboard JS) — a different domain fed by `snapshot.total_artifacts`. No frontend work.
