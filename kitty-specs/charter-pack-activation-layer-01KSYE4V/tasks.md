...
- T046: In DRG tactic resolution, when `_node_is_activated()` returns `False` for an explicitly requested artifact (one that was directly referenced, not incidentally traversed), raise a hard error naming the artifact and its kind, rather than silently skipping it. Use a dedicated `ArtifactNotActivatedError` exception class.
- T047: Tests cover: (a) `finalize-tasks` raises on unactivated agent profile; (b) `finalize-tasks` passes when profile is activated; (c) `implement` raises on unactivated profile; (d) `implement` does not raise when no profile declared; (e) DRG hard-fail raises `ArtifactNotActivatedError` for direct reference to non-activated artifact.
