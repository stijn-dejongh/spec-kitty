...
- **Do NOT** run redirect/link tooling or edit `redirect_map.yaml` / the page-inventory / `toc.yml` — WP05
  owns the derived-artifact regeneration (single-writer). Your job ends at the moves + `index.md` + `related:`
  frontmatter on the files you moved.
...
- **Steps**:
  ...
  3. **If STAYS (default)**: do nothing to the file (leave it in `architecture/`; it will be captured by
     `index.md` in T017). **If MOVE (verified only)**: `git mv` it to
     `docs/plans/engineering-notes/code-as-a-crime-scene-overview.md`, fix `related:`, and note its **2 docs
     referrers** for WP05.
- **Files**: `docs/architecture/assessments/code-as-a-crime-scene-overview.md` (and its engineering-notes
  destination ONLY if verified point-in-time).
...
- **Steps**:
  ...
  3. **If MOVE (verified)**: `git mv`, scrub point-in-time residue, fix `related:`, and note its **4 docs
     referrers** + that `redirect_baseline_urls.json` already carries the closeout URL (WP05 must keep the
     baseline green). **If STAYS**: leave it.
- **Files**: `docs/migrations/teamspace-mission-state-920-closeout.md` (+ engineering-notes destination if
  verified).
- **Validation**: recorded verdict; file location matches verdict.
- **Edge cases**: the wider `migrations/` dated-residue sweep is OUT of scope (only this one named file);
  default STAYS unless verified.
...
- **Steps**:
  1. After T012–T016 settle the section's membership, enumerate every remaining `.md` under
     `docs/architecture/` (excluding the moved-out files; INCLUDING any borderline that STAYED).
  ...
  3. This is exactly what WP02's `index_completeness` check verifies for `architecture/` (the sole
     curated-complete section) — the index must be complete or that check fails in WP05's sweep.
- **Files**: `docs/architecture/index.md`.
- **Validation**: every non-index `.md` in `architecture/` appears in `index.md`; a manual diff of
  `ls docs/architecture/**/*.md` vs the index entries shows full coverage.
- **Edge cases**: do not list moved-out files; do include subdirectory pages if the section convention does;
  do not relocate `index.md` (it stays).
