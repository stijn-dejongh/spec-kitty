...
- Add a `tags: list[str]` field to the **`DRGNode`** model (Pydantic v2 defaults to `extra='ignore'`, so an un-modelled `tags` key is silently dropped on load) so the anti-pattern marker round-trips. The anti-pattern marker's home is `DRGNode` / `NodeKind` / `drg/validator.py` — **not** a per-artifact "node schema" (none exists). Whether anti-pattern targets also get a new `NodeKind` is D2.
...
## Open Decisions (escalated)

- **D1 — downstream/org-pack `opposed_by` compatibility.** [NEEDS CLARIFICATION: removing `opposed_by` from `additionalProperties: false` schemas hard-breaks any downstream/org-pack YAML that authored it (validated by `pack_validator.py`). Accept the hard break — justified if `opposed_by` was built-in-only and never a documented pack-author surface — and record it, or add a deprecation window / consumer migration path?] <!-- decision_id: 01KY1XHCKPP44C33P5BJ7QJBPV -->
- **D2 — anti-pattern/smell node kind.** [NEEDS CLARIFICATION: do `rejects` targets get a new `NodeKind` (e.g. `anti_pattern`/`smell`) — rippling through `ArtifactKind`, `_SINGULAR_TO_PLURAL`, the activation filter, and schemas — or stay `kind: paradigm`/`tactic` nodes carrying only a `tags` anti-pattern marker?] <!-- decision_id: 01KY1XHDJC35VFFFHH5S10G2Y7 -->
- **D3 — tension check under implicit all-active activation.** [NEEDS CLARIFICATION: `run_consistency_check` short-circuits to `coherent=true` when there is no explicit activation list (backward-compat all-active). Should the tension check run under implicit all-active too — making FR-011 load-bearing and SC-002 a live assertion — or only under explicit activation (in which case the default-pack fixture must explicitly activate 024/025)?] <!-- decision_id: 01KY1XHEH2T9RDX8ZCHCSV2VA0 -->
...
| FR-004 | Anti-pattern/smell node marker | As a maintainer, I want anti-pattern/smell targets to be first-class nodes marked with a `tags` attribute containing `anti-pattern` or `smell` (single pinned attribute + token set), modelled on `DRGNode`, so every `rejects` edge terminates at a validatable node. Node-kind treatment per D2. | High | Open |
...
| FR-009 | Advisory `tension_unreconciled` finding | As an operator, I want `charter consistency-check` to emit an advisory finding for co-activated unreconciled tension over the activation-filtered graph; the finding is added to `ConsistencyReport` but excluded from the `coherent` boolean; the DRG load fails closed. Behaviour under implicit all-active per D3. | High | Open |
...
- **Anti-pattern/smell node marker**: `DRGNode.tags` attribute making rejection targets first-class and validatable.
...
- **SC-002**: The built-in default pack reports `coherent = true` with 0 unreconciled-tension findings out of the box, AND the reconciliation directive `reconcile-change-scope-tensions` is what clears them — verified by a fixture that explicitly co-activates 024/025 so the assertion is live, not vacuous (contingent on D3).
