...
- **Test surface (DRIFTED UP from the issue's 49): 78 files / ~497 rewrite sites** = 417 plain import lines (mechanical, loud-on-error — a wrong re-point fails at collection) + **80 single-line `patch()` string targets (twice-verified strict count; paula's broader sweep suggests up to ~140 sites once multi-line `patch.dict(sys.modules,...)` constructs are enumerated — the plan-phase occurrence-map ledger is the BINDING enumerator and must capture multi-line constructs; budget to the ledger count)** (the silent-no-op class Wave 1 proved real — per-line verification required, blind sed is a review reject). Dominant modules: `runtime_bridge` (186), `decision` (54), `_internal_runtime.{schema,engine}` (23 each).
