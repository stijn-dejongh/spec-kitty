...
| FR-015 | A mission-type definition may optionally declare a `template_set: {artifact_type: template_id}` mapping. This evolves the existing string-valued `template_set` field (e.g., `template_set: software-dev-default`) into an explicit per-artifact dict (e.g., `template_set: {spec: spec-template.md, plan: org-custom-plan.md}`). Each declared template ID is resolved from the active `built-in → org → project` layer chain. The existing ATDD suite (`test_wp_prompt_governance_contract.py`) is migrated from the string form to the dict form as part of this mission. | P2 | Proposed |
