...
| FR-007 | `spec-kitty next` reads `action_sequence` from the resolved mission-type profile for the active mission and dispatches accordingly. The sequence is never hardcoded in the runtime. | P1 | Proposed |
...
| FR-009 | A user can create a **custom mission type** with a unique `id` not present in the shipped set. `spec-kitty mission create` accepts `--mission-type <id>` for any registered type. | P1 | Proposed |
...
| C-004 | DDD boundary: the Doctrine bounded context (`src/doctrine/`) owns all mission-type definitions and step templates. The `specify_cli` runtime (`src/specify_cli/`) consumes them through the doctrine resolver as an anti-corruption layer. `spec-kitty next` must not read charter YAML or mission-type YAML directly. | Binding |
