...
Spec Kitty's doctrine layer is composable for directives, tactics, and agent profiles — but the mission workflow sequence and the step prompts that drive agents are still hardcoded in the CLI. Teams whose real process differs from the shipped `specify → plan → tasks → implement → review → merge` sequence, or whose organisation requires custom governance layers, currently have no extension point short of forking the codebase.
...
3. **Templates DRG layer** — shipped doctrine templates are enumerable and overridable via the standard resolution chain (#1333)
...
| Spec Kitty contributor | Maintains shipped doctrine artifacts in `src/doctrine/` |
...
|---|---|---|---|
| FR-001 | `org-charter.yaml` supports an optional `extends: <pack-name>` key. When present, `required_directives` and `required_toolguides` are resolved as the **union** of base and overlay (overlay can add, never remove). `interview_defaults` are resolved per-key (overlay key wins, unmentioned keys inherit from base). | P1 | Proposed |
...
| FR-004 | Each mission type (`software-dev`, `documentation`, `research`, `plan`) is a first-class governed artifact in the doctrine layer, resolved via the `shipped → org → project` chain. Governance is keyed from `mission_type` in `meta.json`, not from `template_set`. | P1 | Proposed |
...
| FR-006 | Non-software missions (`documentation`, `research`, `plan`) resolve only their own mission-type governance. Software-dev directives are not injected unless the mission-type definition explicitly references them. | P1 | Proposed |
...
| FR-008 | A project-level or org-level `mission-type` override can **extend** a shipped type (adding or reordering steps) or **replace** it entirely (no `extends:` key → full replacement). | P1 | Proposed |
| FR-009 | A user can create a **custom mission type** with a unique `id` not present in the shipped set. `spec-kitty mission create` accepts `--mission-type <id>` for any registered type. "Registered" means present in the set returned by `charter.existing_mission_types(repo_root)`, which enumerates all mission-type IDs visible in the active `shipped → org → project` DRG inventory. Validation occurs at `mission create` time; an unknown `--mission-type` raises `UnknownMissionTypeError` with the queried ID and the list of registered IDs before the mission is created. | P1 | Proposed |
| FR-010 | All step prompt templates that are currently hardcoded in `src/specify_cli/missions/*/command-templates/` become doctrine `mission-step` artifacts under `src/doctrine/missions/mission-steps/`. The shipped set encodes the current prompts verbatim as the migration baseline. | P1 | Proposed |
| FR-011 | A `mission-step` artifact defines: `id`, `display_name`, `step_type` (`agent` \| `human_in_loop` \| `integration`), and `prompt_template` (path to a Markdown template within the same resolution layer). | P1 | Proposed |
| FR-012 | Org and project layers can override `mission-step` prompt templates by providing a file at the corresponding path in their pack or `.kittify/overrides/` directory. Resolution follows the standard `shipped → org → project` precedence. | P1 | Proposed |
| FR-013 | `spec-kitty template list [--kind <mission\|diagram\|checklist\|…>]` enumerates shipped doctrine templates by category. Output includes name, kind, source layer, and resolution path. | P2 | Proposed |
| FR-014 | The template DRG layer follows `shipped → org → project` resolution. An org or project layer can shadow a shipped template by providing a file with the same `id` in their pack. | P2 | Proposed |
...
| NFR-002 | Zero migration required for existing software-dev missions. The shipped `software-dev` mission-type definition encodes the current action sequence; `spec-kitty next` behaviour is identical before and after the change. | 0 breaking changes | Proposed |
| NFR-003 | Mission-type and mission-step resolution must be covered by tests at the shipped-only, org-override, and project-override layers. Extends chains of depth ≥ 2 must be tested. | 100% path coverage | Proposed |
...
| C-006 | Shipped mission-type definitions are immutable at runtime. Org and project overrides shadow; they do not mutate the shipped artifacts. | Binding |
...
| **Mission-type resolution chain** | The `shipped → org → project` precedence chain for resolving the active mission-type definition | "override chain", "lookup order" |
...
| `MissionStep` | `id`, `display_name`, `step_type: agent\|human_in_loop\|integration`, `prompt_template: path` | Entity owned by MissionType |
...
1. A team can add a custom step to the software-dev workflow in their project `.kittify/` without touching shipped code, and `spec-kitty next` dispatches that step after merge.
...
6. All shipped step prompt templates are resolvable through the doctrine layer; org and project overrides can shadow individual step prompts.
7. `spec-kitty template list` returns at least the shipped templates with correct kind and source-layer metadata.

---

## Assumptions

- The `src/doctrine/` package structure and the `shipped → org → project` DRG resolution pattern established by #832 are the baseline; this mission extends them to `mission-types` and `mission-steps`.
...
- `charter.existing_mission_types(repo_root: Path) -> list[str]` is a new public entrypoint added to `src/charter/mission_type_profiles.py`. It returns the deduplicated, sorted list of mission-type IDs registered across all active DRG layers (shipped + org + project). It is the single source of truth for "what mission types exist" at CLI validation time.
