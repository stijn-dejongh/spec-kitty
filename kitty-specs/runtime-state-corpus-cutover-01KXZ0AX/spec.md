...
| C-011 | Re-seed under a NEW field namespace | The resolved-binding re-seed (IC-08) MUST mint seeds under a **new** `_seed_id(mission_id, wp_id, "resolved_binding")` namespace — NOT by mutating the already-committed `"claim"` seed. The idempotency check drops any id already on disk, so reusing `"claim"` would make the re-run silently skip and leave resolved slots empty for the whole dogfood corpus. | Technical | High | Open |
