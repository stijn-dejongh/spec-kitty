...
| FR-007 | Integrity guard | As a maintainer, I want a test that pins each entry's `source_module` against its retained audit comment (and/or a known corpus module), replacing the deleted `_recover_provenance` cross-check, so provenance drift cannot silently move onto the field. | High | Open |
