...
| FR-010 | SaaS-faithful genesis seed | US5 · the `genesis → planned` seed is emitted to SaaS as the bootstrap-planned representation (`from_lane=None`), not dropped; no swallowed `ValidationError`. | Medium | Open |
| FR-011 | SaaS lane validator single-sourced | US5 · `sync/emitter.py` `_PAYLOAD_RULES["WPStatusChanged"]` lane set is derived from the canonical lane source, not a parallel hardcoded list. | Medium | Open |
| FR-012 | FSM force-override parity or explicit boundary | US6 · `transition_to` honours `ctx.force` (actor+reason) for terminal exits, OR the ABC docstring + ADR document `validate_transition` as the force authority, pinned by a test. | Medium | Open |
...
| C-004 | spec_kitty_events boundary | Prefer representing genesis to SaaS via the existing `from_lane=None` bootstrap-planned contract over an upstream enum change; if an upstream change is needed, file a coordinated gap. | Technical | Medium | Open |
