...
- 2026-07-27T00:00:00Z – claude – **Implemented; moved to `for_review`.** One root cause, not five bugs: `merge.py` ran two asymmetric endpoint policies (source had to be fragment-local, miss → silently dropped; target fell back to `directive:<id>`, miss → invented kind), and neither accepted the URN form the pack's own emitter produces. Replaced by one ordered precedence in `_resolve_edge_endpoint`: fragment-local bare id → fully-qualified URN → bare id against the built-in layer recovering the *declared* kind → typed `OrgDRGConflict` hard fail.
