...
| FR-003 | One shared allow/collision predicate, two enforcement points | As a maintainer, I want a single pure predicate (inputs: current holder actor+role, requesting actor+role) shared by both the state-machine guard and the dedicated-review re-claim check, so the two claim paths cannot drift and both are role-aware. | High | Open |
| FR-004 | Thread role end-to-end from one reduction | As a maintainer, I want `role` carried from the reduced status slot through the transaction-resolved read to every guard-construction site, so the guard sees role without a second read. Enumerated: widen the transactional read to return actor+role (prefer a frozen value object over a positional tuple); update all consumers; add `current_role` to the guard input contract (`GuardContext`, `TransitionContext`, `TransitionInputs`); populate it at **every** guard-construction site (both the move-task construction and `_prepare_event`). | High | Open |
