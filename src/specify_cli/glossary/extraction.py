...
def extract_metadata_hints(metadata: Dict[str, Any]) -> List[ExtractedTerm]:
    ...
    return [
        ExtractedTerm(
            surface=term, source="metadata_hint", confidence=1.0, original=term
        )
...
def extract_quoted_phrases(text: str) -> List[ExtractedTerm]:
    ...
    return [
        ExtractedTerm(
            surface=term, source="quoted_phrase", confidence=0.8, original=term
        )
        for term in sorted(terms)
    ]
...
def extract_acronyms(text: str) -> List[ExtractedTerm]:
    ...
    return [
        ExtractedTerm(
            surface=term, source="acronym", confidence=0.8, original=term.upper()
        )
        for term in sorted(terms)
    ]
...
def extract_casing_patterns(text: str) -> List[ExtractedTerm]:
    ...
    return [
        ExtractedTerm(
            surface=term, source="casing_pattern", confidence=0.8, original=term
        )
        for term in sorted(terms)
    ]
...
def extract_repeated_nouns(text: str, min_occurrences: int = 3) -> List[ExtractedTerm]:
    ...
    return [
        ExtractedTerm(
            surface=word, source="repeated_noun", confidence=0.5, original=word
        )
