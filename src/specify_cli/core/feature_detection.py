...
class FeatureContext:
    ...
    @classmethod
    def from_slug(
        cls, slug: str, repo_root: Path, detection_method: str
    ) -> "FeatureContext":
    ...
    # Priority 5: Single feature auto-detect
        ...
        if len(all_features) == 1:
                ...
                if latest:
                    ...
                    if announce_fallback:
                        try:
                            ...
                            console = Console()
                            console.print(
                                f"[yellow]ℹ️  Auto-selected latest incomplete: {latest}[/yellow]"
                            )
                        except ImportError:
            ...
            if not detected_slug:
                ...
                kitty_specs_dir = main_repo_root / "kitty-specs"
                all_complete = all(
                    is_feature_complete(kitty_specs_dir / slug) for slug in all_features
                )
