...
def run_diagnostics(project_dir: Path) -> Dict[str, Any]:
    ...
    diagnostics["file_integrity"] = {
        ...
        "missing_files": list(file_check["missing"].keys())
    ...
    if (
        current_feature.get("detected")
        and current_feature.get("state") == "in_development"
    ):
    ...
    if worktree_summary.get("active_worktrees", 0) > 5:
        observations.append(
            f"Multiple worktrees active: {worktree_summary['active_worktrees']}"
        )
    ...
    if dashboard_file.exists():
            ...
            if port:
                ...
                if not is_healthy:
                    diagnostics["issues"].append(
                        f"Dashboard metadata exists but not responding on port {port}"
                    )
                    if pid:
        ...
        try:
            ...
            url, port, started = ensure_dashboard_running(
                project_dir, background_process=False
            )
