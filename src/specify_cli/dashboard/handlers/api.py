...
class APIHandler(DashboardHandler):
    ...
    def handle_root(self) -> None:
        ...
        try:
            ...
            if active_feature:
                feature_mission_key = active_feature.get("meta", {}).get(
    ...
    def handle_dossier(self, path: str) -> None:
        ...
        if not feature_slug:
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"error": "Missing feature parameter"}).encode()
            )
            return
        ...
        try:
            ...
            if path == "/api/dossier/overview":
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"error": "Dossier endpoint not found"}).encode()
                )
                return
