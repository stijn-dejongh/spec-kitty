...
def realistic_feature_dir(tmp_path):
    ...
    # Evidence artifacts
    (feature_dir / "test_main.py").write_text(
        "# Test\ndef test_x(): pass\n" * 10, encoding="utf-8"
    )
    (feature_dir / "test_api.py").write_text(
        "# API Test\ndef test_api(): pass\n" * 10, encoding="utf-8"
    )
    ...
    design_dir.mkdir()
    for i in range(1, 4):
        (design_dir / f"diagram{i}.txt").write_text(
            f"Diagram {i} content\n" * 5, encoding="utf-8"
        )
        ...
        if len(missing) > 0:
            ...
            for artifact in missing:
                assert artifact.error_reason == "not_found", (
                    f"Expected not_found, got {artifact.error_reason}"
                )
...
class TestOptionalArtifactHandling:
    ...
    def test_optional_artifacts_not_required(self, minimal_feature_dir):
        ...
        dossier = indexer.index_feature(
            minimal_feature_dir, "software-dev", step_id="planning"
        )
        ...
        assert len(missing_required) == 0, (
            f"Should have no missing required, got {len(missing_required)}"
        )
...
class TestUnreadableArtifactHandling:
    ...
    def test_permission_denied_artifact_recorded(self, tmp_path):
        ...
        try:
            ...
            secret_artifact = next(
                (a for a in dossier.artifacts if "secret" in a.relative_path), None
            )
    ...
    def test_invalid_utf8_artifact_recorded(self, tmp_path):
        ...
        corrupted = next(
            (a for a in dossier.artifacts if "corrupted" in a.relative_path), None
        )
...
class TestFullScanWorkflowIntegration:
    ...
    def test_full_workflow_integration(self, realistic_feature_dir):
        ...
        indexer = Indexer(ManifestRegistry())
        dossier = indexer.index_feature(
            realistic_feature_dir, "software-dev", step_id="planning"
        )
...
class TestDeepNestingAndSpecialCharacters:
    ...
    def test_deeply_nested_directories(self, tmp_path):
        ...
        assert len(dossier.artifacts) >= 32, (
            f"Expected >=32 artifacts, got {len(dossier.artifacts)}"
        )
    ...
    def test_special_characters_in_filenames(self, tmp_path):
        ...
        (feature_dir / "file_with_underscores.md").write_text(
            "# Underscores\n", encoding="utf-8"
        )
...
class TestConcurrentFileModification:
    ...
    def test_snapshot_point_in_time_capture(self, tmp_path):
        ...
        spec_artifact = next(
            (a for a in dossier.artifacts if "spec" in a.artifact_key), None
        )
...
class TestEdgeCasesCombined:
    ...
    def test_empty_directory_scan(self, tmp_path):
        ...
        assert dossier.manifest is not None
        assert (
            dossier.completeness_status == "complete"
        )  # No required_always artifacts, so complete
